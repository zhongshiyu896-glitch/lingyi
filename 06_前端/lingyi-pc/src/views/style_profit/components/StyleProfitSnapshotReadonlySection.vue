<template>
  <section class="readonly-section" data-testid="cand412-style-profit-readonly-section">
    <div class="readonly-header">
      ...
      <div class="tag-list" data-testid="cand412-style-profit-readonly-tags">
        <el-tag
          ...
          :type="tag.type"
          effect="plain"
          :data-testid="`cand412-style-profit-tag-${tag.key}`"
        >
          {{ tag.label }}
        </el-tag>
      </div>
    ...
    <el-alert
      type="info"
      :closable="false"
      :title="`snapshot-readonly query state: ${summary.readonlySourceLabel}`"
      :description="`parity=${summary.parityLabel}; focus=${summary.focusStateLabel}; filters=${summary.queryStateLabel}`"
      data-testid="cand412-style-profit-query-state"
    />

    <div class="marker-grid" data-testid="cand412-style-profit-key-markers">
      <div class="marker-card" data-testid="cand412-style-profit-marker-parity">
        <span class="marker-label">style-profit parity</span>
        <strong class="marker-value">{{ summary.parityLabel }}</strong>
      </div>
      <div class="marker-card" data-testid="cand412-style-profit-marker-status">
        <span class="marker-label">snapshot rows/items</span>
        <strong class="marker-value">{{ summary.itemStatusLabel }}</strong>
      </div>
      <div class="marker-card" data-testid="cand412-style-profit-marker-focus">
        <span class="marker-label">source-map focus</span>
        <strong class="marker-value">{{ summary.focusStateLabel }}</strong>
      </div>
      <div class="marker-card" data-testid="cand412-style-profit-marker-source">
    ...
    <el-alert
      ...
      data-testid="cand412-style-profit-blocked-reason"
    />
    <el-alert
      type="warning"
      :closable="false"
      ...
      data-testid="cand412-style-profit-readonly-guard"
    />
    <el-alert
      type="info"
      :closable="false"
      ...
      data-testid="cand412-style-profit-remaining-gap"
    />

    <div class="metric-grid" data-testid="cand412-style-profit-metrics">
      <div
        v-for="metric in summary.metrics"
        :key="metric.key"
        class="metric-card"
        :data-testid="`cand412-style-profit-metric-${metric.key}`"
      >
    ...
    <el-descriptions
      ...
      data-testid="cand412-style-profit-summary"
    >
      <el-descriptions-item label="快照摘要">{{ summary.snapshotSummary }}</el-descriptions-item>
      <el-descriptions-item label="style-profit parity">{{ summary.parityLabel }}</el-descriptions-item>
    ...
    <div class="item-list" data-testid="cand412-style-profit-items">
      <details
        v-for="item in summary.items"
        :key="item.key"
        class="item-card"
        :open="item.key === summary.items[0]?.key"
        :data-testid="`cand412-style-profit-item-${item.key}`"
      >
    ...
    <div class="guarded-action-list" data-testid="cand412-style-profit-guarded-actions">
      <div
        v-for="action in summary.guardedActions"
        :key="action.key"
        class="guarded-action-card"
        :data-testid="`cand412-style-profit-guarded-action-${action.key}`"
      >
...
import type { StyleProfitSnapshotReadonlySectionSummary } from '@/views/style_profit/composables/useStyleProfitSnapshotReadonly'

defineProps<{
  summary: StyleProfitSnapshotReadonlySectionSummary
}>()
</script>
