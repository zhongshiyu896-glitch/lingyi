<template>
  <div class="workshop-daily-wage" data-testid="cand437-workshop-daily-wage-shell">
    <el-card shadow="never" data-testid="workshop-daily-wage-main-section">
      ...
      <el-alert
        ...
        :closable="false"
        show-icon
        :title="dailyWageParityHint"
        data-testid="cand437-workshop-daily-wage-parity-hint"
      />

      <section
        class="cross-route-readonly"
        data-testid="cand437-workshop-daily-wage-cross-route-guard"
      ...
      >
        <div class="cross-route-readonly__main">
          ...
          <span>日工资统计 / production-order / daily-wage-source 共享只读边界，当前来源：/workshop/daily-wages。</span>
        </div>
        <div class="cross-route-readonly__meta">
          <span>日工资筛选、合计与确认/导入/导出入口仅做只读可见验收。</span>
          <span>禁止真实工资确认、ERPNext、outbox、worker 与跨模块执行成功。</span>
        </div>
        <div class="cross-route-readonly__guards" data-testid="cand437-workshop-daily-wage-guarded-entry-list">
          <el-tag type="info" effect="plain">日工资确认 guarded</el-tag>
          <el-tag type="info" effect="plain">批量导入 guarded</el-tag>
          <el-tag type="info" effect="plain">导出 guarded</el-tag>
          <el-tag type="info" effect="plain">ERPNext guarded</el-tag>
          <el-tag type="info" effect="plain">Outbox guarded</el-tag>
          <el-tag type="info" effect="plain">Worker guarded</el-tag>
          <el-tag type="info" effect="plain">跨模块执行 guarded</el-tag>
        </div>
      </section>
      ...
      <template v-else>
        ...
        <template v-else>
          ...
          />
          <div class="guarded-actions" data-testid="cand437-workshop-daily-wage-page-guarded-actions">
            <span
              hidden
            ...
            <el-button
              ...
              data-write-guard="readonly:workshop-daily-wage-confirm"
              data-guard-state="guarded_readonly"
              :disabled="true"
              :title="guardedActionMap['daily-wage-confirm'].reason"
              aria-disabled="true"
              @click="guardedWriteAction('日工资确认')"
            >
              确认
            </el-button>
            <el-button
              data-action-type="write"
              data-write-guard="readonly:workshop-daily-wage-import"
              data-guard-state="guarded_readonly"
              :disabled="true"
              :title="guardedActionMap['daily-wage-import'].reason"
              aria-disabled="true"
              @click="guardedWriteAction('批量导入')"
            >
              导入
            </el-button>
            <el-button
              ...
              aria-disabled="true"
              @click="guardedWriteAction('导出日工资')"
            >
              导出
            </el-button>
          </div>
...
const dailyWageCurrentPath = computed<string>(() => route.fullPath || '/workshop/daily-wages')
const dailyWageParity = computed<string>(() => String(route.query.parity || 'production-order'))
const dailyWageFocus = computed<string>(() => String(route.query.focus || 'daily-wage-source'))
const dailyWageParityHint = computed<string>(() => (
  `当前页面为只读验证模式（parity=${dailyWageParity.value} / focus=${dailyWageFocus.value}）`
))
const readonlySummary = useWorkshopDailyWageReadonly({
  ...
  canRead,
  currentPath: dailyWageCurrentPath,
  parity: dailyWageParity,
  focus: dailyWageFocus,
})
