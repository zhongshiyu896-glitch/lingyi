          ...
          <el-table-column label="操作" width="130" fixed="right">
            <template #default="scope">
              <el-button
                ...
                @click="onGuardedDeactivate(scope.row)"
              >
                停用
              </el-button>
            </template>
...
import { fetchWorkshopWageRates, type WorkshopWageRateRow } from '@/api/workshop'
...
const onGuardedCreate = (): void => {
  if (!canManage.value) {
    ElMessage.warning('无工价维护权限')
    return
  }
  ElMessage.warning('当前阶段仅支持只读验证，新增工价已禁用')
}

const onGuardedDeactivate = (row: WorkshopWageRateRow): void => {
  if (!canManage.value || row.status !== 'active' || (row.is_global && !canManageAll.value)) {
    ElMessage.warning('当前记录不可停用或无权限')
    return
  }
  ElMessage.warning('当前阶段仅支持只读验证，停用工价已禁用')
}
