...
export interface LocalBomLinePayload {
  material_item_code: string
  material_name: string
  style_code: string
  color: string
...
export interface LocalBomOperationPayload {
  process_name: string
  sequence_no: number
  is_subcontract: boolean
  wage_rate: number
...
export interface LocalBomReadbackFlags {
...
export interface LocalBomReadbackData {
  object_id: number
  draft_id: number
  scenario_tag: string
  bom_main: {
  ...
  fabric_lines: LocalBomLinePayload[]
  trim_lines: LocalBomLinePayload[]
  operations: LocalBomOperationPayload[]
  readback_flags: LocalBomReadbackFlags
  state: string
  created_at: string
  updated_at: string
}

export interface LocalBomListSummaryData {
  scenario_tag: string
  total: number
  records: LocalBomReadbackData[]
}

export interface LocalBomRollbackData {
  scenario_tag: string
  object_id: number
...
export interface LocalBomResidualData {
  scenario_tag: string
  total: number
}

export interface LocalBomUpsertPayload {
  scenario_tag: string
  bom_main: {
    bom_no: string
    item_code: string
...
const LOCAL_BOM_ENDPOINT = '/api/local-dev/bom'
const LOCAL_BOM_LIST_ENDPOINT = '/api/local-dev/bom/list'
const LOCAL_BOM_RESIDUAL_ENDPOINT = '/api/local-dev/bom/residual-count'
...
export const createLocalBom = (payload: LocalBomUpsertPayload): Promise<ApiResponse<LocalBomReadbackData>> =>
  request(LOCAL_BOM_ENDPOINT, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  })

export const updateLocalBom = (
  objectId: number,
  payload: LocalBomUpsertPayload,
): Promise<ApiResponse<LocalBomReadbackData>> =>
  request(`${LOCAL_BOM_ENDPOINT}/${objectId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  })

export const fetchLocalBomReadback = (
  objectId: number,
  scenarioTag: string,
): Promise<ApiResponse<LocalBomReadbackData>> =>
  request(`${LOCAL_BOM_ENDPOINT}/${objectId}/readback?${toQuery({ scenario_tag: scenarioTag })}`)

export const rollbackLocalBomScenario = (
  objectId: number,
  scenarioTag: string,
): Promise<ApiResponse<LocalBomRollbackData>> =>
  request(`${LOCAL_BOM_ENDPOINT}/${objectId}/rollback`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ scenario_tag: scenarioTag }),
  })

export const fetchLocalBomResidualCount = (
  scenarioTag: string,
): Promise<ApiResponse<LocalBomResidualData>> =>
  request(`${LOCAL_BOM_RESIDUAL_ENDPOINT}?${toQuery({ scenario_tag: scenarioTag })}`)

export const fetchLocalBomListSummary = (
  scenarioTag: string,
): Promise<ApiResponse<LocalBomListSummaryData>> =>
  request(`${LOCAL_BOM_LIST_ENDPOINT}?${toQuery({ scenario_tag: scenarioTag })}`)
