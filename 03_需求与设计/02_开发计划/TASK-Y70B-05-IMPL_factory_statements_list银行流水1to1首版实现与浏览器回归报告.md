# TASK-Y70B-05-IMPL /factory-statements/list 银行流水 1:1 首版实现与浏览器回归报告

## 1. 任务与边界
- TASK_ID: `TASK-Y70B-05-IMPL`
- 路由: `/factory-statements/list`
- 目标语义: `TASK-Y69B-P1-05 银行流水`（只读首版）
- allowlist: `TASK-Y70B-05` baseline 冻结的 5 个 `/factory-statements/list` 承载文件
- 约束: 仅 GET 只读；写动作与上传/下载/导出/打印均 guarded，不触发真实写请求

## 2. 实现摘要
- 前端页面（`FactoryStatementList.vue`）新增“银行流水（TASK-Y69B-P1-05）”只读区块：
  - `data-testid="bank-ledger-section"`
  - 筛选项：流水单号、关联业务单号、银行、账户名称、交易类型、流水状态、复核状态、关键词、开始/结束日期
  - 表格字段：流水单号、关联业务单号、公司、银行、账户名称、账号、币种、交易类型、借方金额、贷方金额、交易后余额、流水状态、复核状态、流水日期、凭证号、经办人、备注
  - 操作按钮：入账确认提示、复核提示、流水校验、导出、打印，全部为 guarded/提示型
  - 新增 `loadBankLedgers`、`resetBankLedgerFilters`、`onBankLedgerPageChange`、`onBankLedgerSizeChange`，并在 `onMounted` 接入加载
- 前端 API（`factory_statement.ts`）新增并对接：
  - `fetchFactoryStatementBankLedgers`
  - `GET /api/factory-statements/bank-ledgers`
  - 对应 query/item/data 类型保持只读
- 后端 router（`factory_statement.py`）新增只读路由：
  - `GET /api/factory-statements/bank-ledgers`
  - 权限边界保持 `FACTORY_STATEMENT_READ`
  - 静态路由位于动态 `/{statement_id}` 之前（`2529 < 2648`，route shadowing guard 生效）
- 后端 schema/service 新增只读聚合：
  - schema: `FactoryStatementBankLedgerItem` / `FactoryStatementBankLedgerData`
  - service: `get_bank_ledgers(...)`
...
- `Y70B-04` 供应商应付账款汇总表 preserved: PASS
...
- result_json: `/tmp/task_y70b_05_impl_20260506T141714Z_browser_results.json`
- screenshots_dir: `/tmp/task_y70b_05_impl_20260506T141714Z_screenshots`
- screenshots_count: `6`
- 核心断言:
  - `route_open=true`
  - `first_screen_visible=true`
  - `filters_present=true`
  - `bank_ledger_fields_mapped=true`
  - `buttons_mapped=true`
