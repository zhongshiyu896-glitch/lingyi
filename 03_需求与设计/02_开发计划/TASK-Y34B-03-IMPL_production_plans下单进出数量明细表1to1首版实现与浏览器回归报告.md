# TASK-Y34B-03-IMPL `/production/plans` 下单进出数量明细表 1:1 首版实现与浏览器回归报告

## 1. 任务与范围
- TASK_ID: `TASK-Y34B-03-IMPL`
- 目标路由: `/production/plans`（shared route）
- 实现语义: `TASK-Y33B-P1-03` 下单进出数量明细表（只读）
- 前置基线: `task_y34b_03_production_plans_order_io_quantity_allowlist_baseline.json`
- 允许改动: `ProductionPlanList.vue`、`production.ts`、`production.py`、`schemas/production.py`、`production_service.py`
- 未触碰: 非 `/production/plans` 路由、`TASK-Y33B-P1-04/05`、测试/控制面/GitHub/生产配置

## 2. 实现内容（只读）
### 2.1 前端（`ProductionPlanList.vue`）
- 新增区块: `下单进出数量明细表（P1）`
- 新增筛选项:
  - 下单单号、订单行号、款号、客户、关键字、进出状态、生产状态、开始时间、结束时间
- 新增展示字段:
  - 制单号、下单单号、订单行号、客户、款号、下单数量、入库数量、出库数量、待入库、待出库、入库进度、出库进度、进出状态、生产状态、更新时间
- 新增按钮语义（全部受控）:
  - 查看、打印、导出、同步、筛选、清空、列设置
- 新增状态:
  - 空态（暂无下单进出数量明细，请调整筛选条件后重试）
  - 错误态（下单进出数量明细加载失败）
  - 权限/禁用态（写动作禁用 + guarded 提示）

### 2.2 前端 API（`production.ts`）
- 新增只读 contract:
  - `ProductionOrderIOQuantityListQuery`
  - `ProductionOrderIOQuantityListItem`
  - `ProductionOrderIOQuantityListData`
- 新增只读调用:
  - `fetchProductionOrderIOQuantities(...) -> GET /api/production/order-io-quantities`

### 2.3 后端（`production.py` / `schemas/production.py` / `production_service.py`）
- 新增只读 schema:
  - `ProductionOrderIOQuantityQuery`
  - `ProductionOrderIOQuantityListItem`
  - `ProductionOrderIOQuantityListData`
- 新增只读路由:
  - `GET /api/production/order-io-quantities`
- 新增只读服务:
  - `list_order_io_quantities(...)`
  - 基于 `LyProductionPlan` 聚合下单/入库/出库/待处理与进度字段，未引入写动作
- 路由遮蔽检查:
  - `GET /api/production/order-io-quantities` 位于 `GET /api/production/plans/{plan_id}` 前，未被动态路由遮蔽
...
- 脚本: `/tmp/task_y34b_03_impl_browser_check.mjs`
- result_json: `/tmp/task_y34b_03_impl_20260504T073419Z_browser_results.json`
- screenshots:
  - `/tmp/task_y34b_03_impl_20260504T073419Z_01_overview.png`
  - `/tmp/task_y34b_03_impl_20260504T073419Z_02_order_io_section.png`
  - `/tmp/task_y34b_03_impl_20260504T073419Z_03_guarded_action.png`
  - `/tmp/task_y34b_03_impl_20260504T073419Z_04_error_state.png`
  - `/tmp/task_y34b_03_impl_20260504T073419Z_05_detail_followup_preserved.png`
  - `/tmp/task_y34b_03_impl_20260504T073419Z_06_permission_disabled_state.png`
- screenshots_count: `6`
...
- `order_io_quantity_fields_mapped`: true
- `buttons_mapped`: true
