# TASK-Y55B-01-IMPL /sales-inventory/stock-ledger 客户退货申请 1:1 首版实现与浏览器回归报告

## 1. 执行范围与边界

- TASK_ID: `TASK-Y55B-01-IMPL`
...
- 新增“客户退货申请（TASK-Y54B-P1-01）”只读区块：
  - 筛选项：申请单号、物料编码、仓库、申请状态、复核状态、关键词、开始/结束日期
  - 展示项：申请退货数量、复核通过数量、待处理数量、申请状态、复核状态、申请日期、申请人、关联单据
  - 状态态：空态、错误态、权限禁用态
- 前端 API 新增只读 GET：
  - `fetchSalesInventoryCustomerReturnApplications`
  - `GET /api/sales-inventory/customer-return-applications`
- 后端新增只读查询接口与聚合：
  - router: `GET /api/sales-inventory/customer-return-applications`
  - schema: `CustomerReturnApplicationItem`, `CustomerReturnApplicationData`
  - service: `get_customer_return_applications(...)`
- 路由遮蔽校验：
  - 新增静态路由位于动态 `items/{item_code}` 路由之前，`route_shadowing_guard=PASS`
- 写动作保持 guarded：
  - 申请确认提示、复核提示、校验、导出、打印均为提示/禁用，不触发真实写请求
- 新增写路由：
...
- result_json：`/tmp/task_y55b_01_impl_20260505T120814Z_browser_results.json`
- screenshots_count：`6`
- screenshots_dir：`/tmp/task_y55b_01_impl_20260505T120814Z_screenshots`
...
- customer_return_application_fields_mapped: `true`
- buttons_mapped: `true`
...
- 启动 TASK-Y54B-P1-02/03/04/05: `NO`
- 修改测试代码/控制面/C审计记录: `NO`
