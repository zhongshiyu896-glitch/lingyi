# TASK-Y65B-04-IMPL /factory-statements/list 加工厂评估表 1:1 首版实现与浏览器回归报告

## 1. 任务与边界
- TASK_ID: `TASK-Y65B-04-IMPL`
- 路由: `/factory-statements/list`
- 目标语义: `TASK-Y64B-P1-04 加工厂评估表`（只读首版）
- allowlist: `TASK-Y65B-04` baseline 冻结的 5 个 `/factory-statements/list` 承载文件
- 约束: 仅 GET 只读；写动作与上传/下载/导出/打印均 guarded，不触发真实写请求

## 2. 实现摘要
- 前端页面（`FactoryStatementList.vue`）新增“加工厂评估表（TASK-Y64B-P1-04）”只读区块：
  - `data-testid="factory-evaluation-section"`
  - 筛选项：评估单号、业务单号、加工厂名称、评估人、评分等级、复核状态、关键词、开始/结束日期。
  - 表格字段：评估单号、关联业务单号、公司、供应商、加工厂名称、加工厂编码、评估人、评分、评分等级、复核状态、跟进状态、评估日期、到期日期、经办人、备注。
...
- 后端 schema/service 新增只读聚合：
  - schema: `FactoryStatementFactoryEvaluationItem` / `FactoryStatementFactoryEvaluationData`
  - service: `get_factory_evaluations(...)`
...
- result_json: `/tmp/task_y65b_04_impl_20260506T085154Z_browser_results.json`
- screenshots_count: `6`
- 核心断言:
  - `route_open=true`
  - `first_screen_visible=true`
  - `filters_present=true`
  - `factory_evaluation_fields_mapped=true`
  - `buttons_mapped=true`
