# TASK-Y70B-02-IMPL /factory-statements/list 供应商评估表 1:1 首版实现与浏览器回归报告

## 1. 任务与边界
- TASK_ID: `TASK-Y70B-02-IMPL`
- 路由: `/factory-statements/list`
- 目标语义: `TASK-Y69B-P1-02 供应商评估表`（只读首版）
- allowlist: `TASK-Y70B-02` baseline 冻结的 5 个 `/factory-statements/list` 承载文件
- 约束: 仅 GET 只读；写动作与上传/下载/导出/打印均 guarded，不触发真实写请求

## 2. 实现摘要
- 前端页面（`FactoryStatementList.vue`）新增“供应商评估表（TASK-Y69B-P1-02）”只读区块：
  - `data-testid="supplier-evaluation-section"`
  - 筛选项：评估单号、关联业务单号、供应商、评估人、评分等级、复核状态、关键词、开始/结束日期
  - 表格字段：评估单号、关联业务单号、公司、供应商、供应商编码、评估人、评分、评分等级、复核状态、跟进状态、评估日期、有效截止、经办人、备注
  - 操作按钮：评估确认提示、复核提示、评级校验、导出、打印，全部为 guarded/提示型
  - 新增 `loadSupplierEvaluations`、`resetSupplierEvaluationFilters`、`onSupplierEvaluationPageChange`、`onSupplierEvaluationSizeChange`，并在 `onMounted` 接入加载
- 前端 API（`factory_statement.ts`）使用并对接：
...
- 后端 router（`factory_statement.py`）新增只读路由：
  ...
  - 静态路由位于动态 `/{statement_id}` 之前（`2175 < 2292`，route shadowing guard 生效）
- 后端 schema/service 新增只读聚合：
  - schema: `FactoryStatementSupplierEvaluationItem` / `FactoryStatementSupplierEvaluationData`
  - service: `get_supplier_evaluations(...)`
...
- result_json: `/tmp/task_y70b_02_impl_20260506T120519Z_browser_results.json`
- screenshots_count: `6`
- 核心断言:
  - `route_open=true`
  - `first_screen_visible=true`
  - `filters_present=true`
  - `supplier_evaluation_fields_mapped=true`
  - `buttons_mapped=true`
...
- 未启动 `TASK-Y69B-P1-03/04/05`: PASS
