...
状态：已关闭；原 19 个历史红已被后续修复与全量回归清零，不再阻塞功能主线。

## 2026-06-21 状态刷新

本任务原用于把 2026-06-18 时点的后端 `19 failed` 从功能主线中剥离，避免 BOM、样衣、大货、采购、库存、报表等闭环交付为赶进度“顺手改绿”。

当前后端全量回归已重新验证为：

- 验证命令：在 `/Users/hh/Desktop/领意服装管理系统/07_后端/lingyi_service` 执行 `.venv/bin/python -m pytest --tb=short -q`
- 当前结果：`1528 passed, 13 skipped`
- 当前检查点：`e065b07 fix: keep fastapi read pages off erpnext adapters`
- 结论：原登记的 19 个历史红已清零，本文件仅作为历史红隔离与清理完成记录保留。后续上线前门禁以当前全量 pytest 结果和最新零回退对照为准，不再引用本文件的 2026-06-18 红名单作为未清阻塞项。
