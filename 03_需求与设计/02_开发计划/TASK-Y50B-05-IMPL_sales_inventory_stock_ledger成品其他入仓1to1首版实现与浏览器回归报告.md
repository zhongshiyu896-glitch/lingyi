# TASK-Y50B-05-IMPL /sales-inventory/stock-ledger 成品其他入仓 1:1 首版实现与浏览器回归报告

## 1. 执行范围与边界

- TASK_ID: `TASK-Y50B-05-IMPL`
...
- 新增“成品其他入仓（TASK-Y49B-P1-05）”只读区块：
  - 筛选项：入仓单号、成品编码、入仓仓库、入仓状态、核销状态、关键词、开始/结束日期
  - 展示项：计划入仓、实际入仓、待入仓、入仓状态、核销状态、入仓日期、来源单号、经办人、关联单据
  - 状态态：空态、错误态、权限禁用态
- 前端 API 新增只读 GET：
  - `fetchSalesInventoryFinishedGoodsOtherInbound`
  - `GET /api/sales-inventory/finished-goods-other-inbound`
- 后端新增只读查询接口与聚合：
  - router: `GET /api/sales-inventory/finished-goods-other-inbound`
  - schema: `FinishedGoodsOtherInboundItem`, `FinishedGoodsOtherInboundData`
  - service: `get_finished_goods_other_inbound(...)`
- 写动作保持 guarded：
  - 其他入仓确认提示、其他入仓核销提示、校验、导出、打印均为提示/禁用，不触发真实写请求
...
- 回归脚本：`/tmp/task_y50b_05_impl_browser_check.mjs`
- result_json：`/tmp/task_y50b_05_impl_20260505T094220Z_browser_results.json`
- screenshots_count：`6`
- screenshots_dir：`/tmp/task_y50b_05_impl_20260505T094220Z_screenshots`
...
- finished_goods_other_inbound_fields_mapped: `true`
- buttons_mapped: `true`
