"""TASK-060B create sales payment entry table.

Revision ID: task_060b_create_sales_payment_entry_table
Revises: task_060a_create_delivery_invoice_table
...
revision = "task_060b_create_sales_payment_entry_table"
down_revision = "task_060a_create_delivery_invoice_table"
branch_labels = None
depends_on = None

_SCHEMA_NAME = "ly_schema"
_TABLE = "ly_sales_payment_entry"
...
def _create_table(bind, schema: str | None) -> None:
    ...
    op.create_table(
        ...
        sa.Column("delivery_invoice_id", sa.BigInteger(), nullable=False),
        ...
        sa.CheckConstraint("status IN ('submitted','cancelled')", name="ck_ly_sales_payment_status"),
        sa.CheckConstraint("paid_amount > 0", name="ck_ly_sales_payment_amount_positive"),
        sa.CheckConstraint("allocated_amount > 0", name="ck_ly_sales_payment_allocated_positive"),
        sa.CheckConstraint("outstanding_before >= 0", name="ck_ly_sales_payment_before_nonnegative"),
        sa.CheckConstraint("outstanding_after >= 0", name="ck_ly_sales_payment_after_nonnegative"),
        schema=schema,
    )
...
def _create_indexes(bind, schema: str | None) -> None:
    indexes = [
        ("uk_ly_sales_payment_company_no", ["company", "payment_entry"], True),
        ("uk_ly_sales_payment_company_idem", ["company", "idempotency_key"], True),
        ("uk_ly_sales_payment_company_source", ["company", "source_ref"], True),
        ("idx_ly_sales_payment_invoice", ["company", "sales_invoice"], False),
        ("idx_ly_sales_payment_customer", ["company", "customer"], False),
    ]
...
def downgrade() -> None:
    ...
    schema = _schema_of(bind)
    for index_name in [
        "idx_ly_sales_payment_customer",
        "idx_ly_sales_payment_invoice",
        "uk_ly_sales_payment_company_source",
        "uk_ly_sales_payment_company_idem",
        "uk_ly_sales_payment_company_no",
    ]:
