...
class LyFactoryStatementPayment(Base):
    """FastAPI-native payment entry allocated to one factory statement."""

    __tablename__ = "ly_factory_statement_payment"
    __table_args__ = (
        Index("uk_ly_factory_statement_payment_company_no", "company", "payment_entry", unique=True),
        Index("uk_ly_factory_statement_payment_company_idem", "company", "idempotency_key", unique=True),
        ...
        CheckConstraint("outstanding_after >= 0", name="ck_ly_factory_statement_payment_after_nonnegative"),
        {"schema": "ly_schema", "comment": "FastAPI 原生加工厂对账付款单"},
    )
