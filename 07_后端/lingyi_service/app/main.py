...
def _infer_security_target(request: Request) -> tuple[str, str | None, str | None, str | None]:
    ...
    if path.startswith("/api/workshop"):
        ...
        if "/wage-payments/" in path and path.endswith("/cancel"):
            return "workshop", WORKSHOP_WAGE_PAYMENT_CANCEL, "WorkshopWagePayment", None
    ...
    if path in {"/api/reports/diagnostic", "/api/reports/diagnostic/"}:
        return "report", REPORT_DIAGNOSTIC, "ReportDiagnostic", None
    if path in {"/api/reports/catalog", "/api/reports/catalog/"}:
        return "report", REPORT_READ, "ReportCatalog", None
    if path in {"/api/reports/catalog/export", "/api/reports/catalog/export/"}:
        return "report", REPORT_EXPORT, "ReportCatalogExport", None
    ...
    if path.startswith("/api/sales-inventory"):
        ...
        delivery_cancel_match = re.match(r"^/api/sales-inventory/delivery-invoices/(\d+)/cancel/?$", path)
        if delivery_cancel_match:
            return "sales_inventory", SALES_INVENTORY_WRITE, "DeliveryInvoice", delivery_cancel_match.group(1)
        if path in {"/api/sales-inventory/sales-returns", "/api/sales-inventory/sales-returns/"}:
            if method == "POST":
                return "sales_inventory", SALES_INVENTORY_WRITE, "SalesReturn", None
            return "sales_inventory", SALES_INVENTORY_READ, "SalesReturn", None
        sales_return_cancel_match = re.match(r"^/api/sales-inventory/sales-returns/(\d+)/cancel/?$", path)
        if sales_return_cancel_match:
            return "sales_inventory", SALES_INVENTORY_WRITE, "SalesReturn", sales_return_cancel_match.group(1)
    ...
    if path.startswith("/api/quality"):
        ...
        if path.endswith("/outbox-status"):
