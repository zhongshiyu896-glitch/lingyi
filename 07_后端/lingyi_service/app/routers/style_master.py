...
@router.get("/styles/{style_id}/skus")
def get_style_skus(
    style_id: int,
    request: Request,
    company: str = Query(default="默认公司"),
    ...
    try:
        data = StyleMasterService(session).list_style_skus(style_id=style_id, company=company)
    except AppException as exc:
        return _err(exc)
    return _ok(data)
...
def get_style_size_chart(
    ...
    try:
        data = StyleMasterService(session).get_style_size_chart(style_id=style_id, company=company)
    except AppException as exc:
        return _err(exc)
    return _ok(data)
...
def upsert_style_size_chart(
    style_id: int,
    payload: StyleSizeChartData,
    request: Request,
    company: str = Query(default="默认公司"),
    current_user: CurrentUser = Depends(get_current_user),
    session: Session = Depends(get_db_session),
    ...
    try:
        result = _mutate(
            ...
            current_user=current_user,
            session=session,
            mutate=lambda service: service.update_style_size_chart(
                style_id=style_id,
                company=company,
                payload=payload,
                actor=current_user.username,
...
@router.put("/styles/{style_id}/skus")
def upsert_style_skus(
    ...
    try:
        result = _mutate(
            ...
            current_user=current_user,
            session=session,
            mutate=lambda service: service.upsert_style_skus(
                style_id=style_id,
                payload=payload,
                actor=current_user.username,
            ),
