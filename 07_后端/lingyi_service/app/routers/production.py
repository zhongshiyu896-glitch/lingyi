...
@router.post("/quotes/{quote_id}/copy", response_model=ApiResponse[ProductionQuoteListItem])
def copy_production_quote(
    quote_id: int,
    ...
    try:
        ...
        data = _service(session=session, request=request).copy_quote(
            quote_id=quote_id,
            payload=payload,
            operator=current_user.username,
        )
        ...
        _record_failure_safely(
            ...
            resource_no=resource_no,
            before_data=None,
            after_data={"quote_id": quote_id, "quote_no": payload.quote_no},
            error_code=exc.code,
        )
        return _app_err(exc)
    except Exception as exc:
...
@router.post("/quotes/{quote_id}/void", response_model=ApiResponse[ProductionQuoteListItem])
def void_production_quote(
    quote_id: int,
    ...
    try:
        ...
        data = _service(session=session, request=request).void_quote(
            quote_id=quote_id,
            payload=payload,
            operator=current_user.username,
        )
        ...
        _record_failure_safely(
            ...
            resource_no=resource_no,
            before_data=None,
            after_data={"quote_id": quote_id, "reason": payload.reason},
            error_code=exc.code,
        )
        return _app_err(exc)
    except Exception as exc:
...
@router.post("/plans/{plan_id}/tracking-nodes", response_model=ApiResponse[ProductionTrackingNodeEventData])
def register_production_tracking_node(
    plan_id: int,
    ...
    try:
        ...
        data = service.register_tracking_node_event(
            plan_id=plan_id,
            payload=payload,
            operator=current_user.username,
            request_id=raw_request_id,
...
@router.post("/sales-orders/{sales_order}/material-check", response_model=ApiResponse[ProductionSalesOrderMaterialCheckData])
def material_check_sales_order(
    sales_order: str,
    request: Request,
    payload: ProductionSalesOrderMaterialCheckRequest = Body(...),
    current_user: CurrentUser = Depends(get_current_user),
    session: Session = Depends(get_db_session),
):
    ...
    audit = AuditService(session=session)
    context = AuditContext.from_request(request)
    before_data: dict[str, Any] | None = {"sales_order": sales_order, "warehouse": payload.warehouse}
    ...
    try:
        ...
        service = _service(session=session, request=request)
        scope_company, scope_item_codes = service.resolve_sales_order_material_check_scope(
            sales_order=sales_order,
        ...
        data = service.material_check_sales_order(
            sales_order=sales_order,
            operator=current_user.username,
            payload=payload,
            request_id=raw_request_id,
