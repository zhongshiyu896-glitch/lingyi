...
class BackendContractAssetsTest(unittest.TestCase):
    ...
    def test_task2_task3_contract_handoff_documents_are_present(self) -> None:
        ...
        self.assertEqual(task2_json["scope"], "backend_contract_only_no_frontend_read")
        self.assertEqual(task2_json["backend_catalog_counts"], {"A": 185, "B": 19, "C": 7, "D": 11})
        self.assertEqual(task2_json["page_matrix_counts"]["total_pages"], 48)
        self.assertEqual(task2_json["page_matrix_counts"]["missing_api_path"], 33)
        self.assertIn("contracts/frontend_backend_page_matrix.json", task2_md)
        self.assertIn("A=185、B=19、C=7、D=11", task2_md)
        self.assertIn("闭环必需但当前页面矩阵未覆盖", task2_md)
        self.assertIn("生产权限源是否脱 ERPNext：PARKED", task2_md)
        self.assertIn("库存/财务是否自建：PARKED", task2_md)
        ...
        self.assertIn("ERPNext 权限源 PARKED", task3_md)
        self.assertIn("库存/财务内核 PARKED", task3_md)
