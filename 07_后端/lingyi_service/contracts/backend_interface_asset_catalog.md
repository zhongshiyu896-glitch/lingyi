...
- A 类真实业务接口：267
- B 类 dev/test readiness 只读接口：16
...
| A | GET | `/api/bom/process-requirement-templates` | 是 | 是 | 否 | candidate | id, company, template_type, template_code, name, scene, status, version, nodes | 真实业务只读接口候选 |
| A | POST | `/api/bom/process-requirement-templates` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | PATCH | `/api/bom/process-requirement-templates/{template_id}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/bom/process-requirement-templates/{template_id}/deactivate` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/bom/process-requirement-templates/{template_id}/nodes` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | PATCH | `/api/bom/process-requirement-templates/{template_id}/nodes/{node_id}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/bom/process-requirement-templates/{template_id}/nodes/{node_id}/deactivate` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
...
| A | GET | `/api/bom/size-chart-templates` | 是 | 是 | 否 | candidate | id, company, template_type, template_code, name, scene, status, version, nodes | 真实业务只读接口候选 |
| A | POST | `/api/bom/size-chart-templates` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | PATCH | `/api/bom/size-chart-templates/{template_id}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/bom/size-chart-templates/{template_id}/deactivate` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/bom/size-chart-templates/{template_id}/nodes` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | PATCH | `/api/bom/size-chart-templates/{template_id}/nodes/{node_id}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/bom/size-chart-templates/{template_id}/nodes/{node_id}/deactivate` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
...
| A | GET | `/api/factory-statements/payments` | 是 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
...
| A | POST | `/api/factory-statements/{statement_id}/payments` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | GET | `/api/master-data/{entity_path}` | 是 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
| A | POST | `/api/master-data/{entity_path}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | PATCH | `/api/master-data/{entity_path}/{record_id}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
...
| A | GET | `/api/production/report-suite` | 是 | 是 | 否 | candidate | code, message, data, data.report_key, data.title, data.items, data.total, data.page, data.page_size, data.trend, data.trend.label, data.trend.amount, ... | 真实业务只读接口候选 |
...
| A | GET | `/api/production/tracking-reconciliations` | 是 | 是 | 否 | candidate | code, message, data, data.items, data.items.id, data.items.reconcile_no, data.items.batch_no, data.items.company, data.items.sample_order_id, data.items.sample_no, data.items.style_no, data.items.style_name, ... | 真实业务只读接口候选 |
| A | POST | `/api/production/tracking-reconciliations/generate` | 否 | 是 | 是 | needs_dedicated_write_task | code, message, data, data.batch_no, data.company, data.created_count, data.updated_count, data.matched_count, data.unmatched_count, data.items, data.items.id, data.items.reconcile_no, ... | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
...
| A | POST | `/api/quality/inspections/{inspection_id}/release` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/quality/inspections/{inspection_id}/rework` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
...
| A | GET | `/api/sales-inventory/delivery-invoices` | 是 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
| A | POST | `/api/sales-inventory/delivery-invoices` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
...
| A | GET | `/api/sales-inventory/payment-entries` | 是 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
| A | POST | `/api/sales-inventory/payment-entries` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
...
| A | PATCH | `/api/sales-inventory/sales-orders/drafts/{draft_id}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
...
| A | GET | `/api/sample/orders` | 是 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
| A | POST | `/api/sample/orders` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | PATCH | `/api/sample/orders/{order_id}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/sample/orders/{order_id}/convert-to-bulk` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | GET | `/api/sample/orders/{order_id}/material-bom` | 否 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
| A | PUT | `/api/sample/orders/{order_id}/material-bom` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/sample/orders/{order_id}/material-bom/copy-from-style` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/sample/orders/{order_id}/material-bom/explode` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/sample/orders/{order_id}/reverse` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/sample/orders/{order_id}/seal` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/sample/orders/{order_id}/start-fitting` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/sample/orders/{order_id}/start-patterning` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/sample/orders/{order_id}/submit` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | GET | `/api/sample/orders/{order_id}/tracking-events` | 是 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
| A | POST | `/api/sample/orders/{order_id}/tracking-events` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | GET | `/api/sample/tracking-templates` | 是 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
| A | POST | `/api/sample/tracking-templates` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | PATCH | `/api/sample/tracking-templates/{template_id}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/sample/tracking-templates/{template_id}/deactivate` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/sample/tracking-templates/{template_id}/nodes` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | PATCH | `/api/sample/tracking-templates/{template_id}/nodes/{node_id}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/sample/tracking-templates/{template_id}/nodes/{node_id}/delete` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | GET | `/api/style-master/dictionaries` | 是 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
| A | POST | `/api/style-master/dictionaries` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | PATCH | `/api/style-master/dictionaries/{dictionary_id}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/style-master/dictionaries/{dictionary_id}/deactivate` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | GET | `/api/style-master/style-gallery` | 是 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
| A | POST | `/api/style-master/style-gallery` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | PATCH | `/api/style-master/style-gallery/{gallery_id}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/style-master/style-gallery/{gallery_id}/deactivate` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | GET | `/api/style-master/styles` | 是 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
| A | POST | `/api/style-master/styles` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | PATCH | `/api/style-master/styles/{style_id}` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/style-master/styles/{style_id}/deactivate` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | GET | `/api/style-master/styles/{style_id}/material-bom` | 否 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
| A | PUT | `/api/style-master/styles/{style_id}/material-bom` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
| A | POST | `/api/style-master/styles/{style_id}/material-bom/explode` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
...
| A | GET | `/api/subcontract/return-materials` | 是 | 是 | 否 | candidate | report_no, subcontract_no, company, supplier, item_code, material_item_code, warehouse, issued_qty, theoretical_usage_qty, planned_return_qty, returned_qty, pending_qty, ... | 真实业务只读接口候选 |
...
| A | POST | `/api/warehouse/factory-return-material-report/{report_no}/return-draft` | 否 | 是 | 是 | needs_dedicated_write_task | - | 真实业务写接口候选; 接前端前必须逐项确认落库、审计、幂等 |
...
| A | GET | `/api/warehouse/stock-entry-drafts` | 是 | 是 | 否 | candidate | - | 真实业务只读接口候选 |
