...
| A | POST | `/api/sales-inventory/sales-orders/drafts` | 否 | 是 | 是 | candidate | id, sales_order_no, source_order_ref, company, customer, status, transaction_date, delivery_date, currency, grand_total, idempotency_key, scenario_tag, items.id, items.draft_id, items.style_master_id, items.item_code, items.item_name, items.color, items.size, items.qty, items.rate, items.amount, items.uom, items.warehouse, items.ys_material_calc_state | 大货订单页已接 FastAPI 原生草稿写入；款式主档 style_master_id 已纳入请求/响应字段，仍受零回退门禁约束 |
| A | PATCH | `/api/sales-inventory/sales-orders/drafts/{draft_id}` | 否 | 是 | 是 | candidate | id, sales_order_no, source_order_ref, company, customer, status, transaction_date, delivery_date, currency, grand_total, idempotency_key, scenario_tag, items.id, items.draft_id, items.style_master_id, items.item_code, items.item_name, items.color, items.size, items.qty, items.rate, items.amount, items.uom, items.warehouse, items.ys_material_calc_state | 大货订单页已接 FastAPI 原生草稿更新；款式主档 style_master_id 已纳入请求/响应字段，仍受零回退门禁约束 |
...
| A | GET | `/api/sales-inventory/sales-orders/{name}` | 否 | 是 | 否 | candidate | name, company, customer, transaction_date, delivery_date, status, docstatus, grand_total, currency, ys_material_calc_state, items.name, items.style_master_id, items.item_code, items.item_name, items.color, items.size, items.qty, items.delivered_qty, items.rate, items.amount, items.warehouse, items.delivery_date, items.ys_material_calc_state | FastAPI 原生大货订单详情；legacy dev/local 草稿 items.style_master_id/items.delivered_qty 可为空 |
...
| A | GET | `/api/subcontract/return-materials` | 是 | 是 | 否 | candidate | report_no, subcontract_no, company, supplier, item_code, material_item_code, warehouse, issued_qty, theoretical_usage_qty, planned_return_qty, returned_qty, pending_qty, report_date, source_doc_no, status | 真实业务只读接口候选；由 FastAPI 外发发料/收货事实推导，不读 readiness seed |
